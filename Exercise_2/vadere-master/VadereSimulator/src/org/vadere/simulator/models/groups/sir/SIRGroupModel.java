...
public class SIRGroupModel extends AbstractGroupModel<SIRGroup> {
	...
	private double currentSimulationInSeconds;
	...
	private void initializeGroupsOfInitialPedestrians() {
		...
		if (c.getElements().size() > 0) {
			...
			for(Pedestrian p : c.getElements()) {
				//Assigns free pedestrians to a group. It assigns pedestrian to a free group if it doesn't belong to any group.
				if(p.getGroupIds()==null || p.getGroupIds().isEmpty()){
					assignToGroup(p);
				}else{
					//System.out.println("H22");
					//System.out.println(p.getGroupIds().getFirst());
